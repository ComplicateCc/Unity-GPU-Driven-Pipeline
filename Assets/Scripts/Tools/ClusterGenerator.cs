...
namespace MPipeline
{
    public unsafe static class ClusterGenerator
    {
        ...
        public static void GetCluster(NativeList<Point> pointsFromMesh, NativeList<int> triangles, Bounds bd, out NativeList<CullBox> boxes, out NativeList<Point> points)
        {
        ...
        private static void GetClusterFromVoxel(Voxel[,,] voxels, out NativeList<CullBox> cullBoxes, out NativeList<Point> points, int vertexCount, int voxelSize)
        {
            ...
            points = new NativeList<Point>(clusterCount * PipelineBaseBuffer.CLUSTERCLIPCOUNT, Allocator.TempJob);
            cullBoxes = new NativeList<CullBox>(clusterCount, Allocator.Temp);
            //Collect all full
            for (int i = 0; i < clusterCount - 1; ++i)
            {
                NativeList<Point> currentPoints = new NativeList<Point>(PipelineBaseBuffer.CLUSTERCLIPCOUNT, Allocator.TempJob);
                ...
                {
                    if (j.vertex.x < lessPoint.x) lessPoint.x = j.vertex.x;
                    else if (j.vertex.x > morePoint.x) morePoint.x = j.vertex.x;
                    if (j.vertex.y < lessPoint.y) lessPoint.y = j.vertex.y;
                    else if (j.vertex.y > morePoint.y) morePoint.y = j.vertex.y;
                    if (j.vertex.z < lessPoint.z) lessPoint.z = j.vertex.z;
                    else if (j.vertex.z > morePoint.z) morePoint.z = j.vertex.z;
                }
